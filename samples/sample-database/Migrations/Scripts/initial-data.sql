...
values ('I did a fun thing', 'witty writing here', (select max(Blogs.Id) from Blogs), 'Andy Watt'),
       ('Wild Scenes in lockdown', 'Banana Bread is delicious', (select max(Blogs.Id) from Blogs), 'Carol Vorderman');
...
values ('Vim for fun and profit', 'content', (select max(Blogs.Id) from Blogs), 'Richard Whiteley'),
       ('Are you also stuck in Vim? Send Help', 'content', (select max(Blogs.Id) from Blogs), 'The Queen');